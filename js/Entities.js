...
Actor = function (type, id, x, y, width, height, img, hp, atkSpd) {
	...
	self.draw = function () {
		...
		if (aimAngle >= 45 && aimAngle < 135)  // up
			directionMod = 2;
		else if (aimAngle >= 135 && aimAngle < 225)  // left
			directionMod = 1;
		else if (aimAngle >= 225 && aimAngle < 315)  // down
			directionMod = 0;

		var walkingMode = Math.floor(self.spriteAnimCounter) % 3;

		ctx.drawImage(self.img,
			walkingMode * frameWidth, directionMod * frameHeight, frameWidth, frameHeight,
			x, y, self.width, self.height
		);
	...
	self.updatePosition = function () {
		// if right bumper touches a wall, you cant go right
		var leftBumber = {x: self.x - 40 , y: self.y};
		var rightBumber = {x: self.x + 40 , y: self.y};
		var upBumber = {x: self.x , y: self.y - 16};
		var downBumber = {x: self.x, y: self.y + 64};

		if(Maps.current.isPositionWall(rightBumber)){
			self.x -= 5;
		} else {
		...
		if(Maps.current.isPositionWall(leftBumber)){
			self.x += 5;
		} else {
		...
		if(Maps.current.isPositionWall(downBumber)){
			self.y -= 5;
		} else {
		...
		if(Maps.current.isPositionWall(upBumber)){
			self.y += 5;
		} else {
			if(self.pressingUp)
				self.y -= self.maxMoveSpd;
